...
const { list1, add, markAsComplete, overdue, dueToday, dueLater } = todoList();
...
describe("Todolist Test Suite", () => {
  ...
  test("Should add new todo", () => {
    const todoItemcount = list1.length;
    add({
      title: "new",
      completed: false,
      duedate: new Date().toISOString().slice(0, 10),
    });
    expect(list1.length).toBe(todoItemcount + 1);
  });

  test("Should mark a todo as complete", () => {
    expect(list1[0].completed).toBe(false);
    markAsComplete(0);
    expect(list1[0].completed).toBe(true);
  });
